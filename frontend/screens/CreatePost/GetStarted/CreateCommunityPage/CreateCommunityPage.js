...
const handleCreateComm = (
  ...
  if (!more) {
    const failname = !name;
    if (failname) {
      setNameError("Enter Commmunity Name");
    }

    //Check UserPassword
    const faillink = !link;
    if (faillink) {
      setLinkError("Enter Community linking code");
    }
    ...
    if (failemail) {
      setEmailError("Enter your community email");
    }

    const failbio = !bio;
    if (failbio) {
      setBioError("Enter your community bio");
    }
