...
import { CreateOrg } from "./CreateOrganizationPage";
...
const CreateOrganizationPages = () => {
  ...
  const handleCreate = () => {
    ...
    if (name.length >= 20) {
      setIsFailed(true);
      setIsFailedError("organization name too long");
    } else if (
      directoryList.some(
        (name) =>
      ...
      setIsFailed(true);
      setIsFailedError("found an organization with the same name");
    } else if (user.verified === false) {
      setIsFailed(true);
      setIsFailedError(
        "You are not verified. This page is available for premium accounts. Get verified."
      );
    } else if (user.directory_status1 !== "Staff") {
      setIsFailed(true);
      setIsFailedError("You are not eligible to create an organization");
    } else if (name.length <= 5) {
      setIsFailed(true);
      setIsFailedError("Organization name too short");
      ...
      setIsFailed(true);
      setIsFailedError("Provide your organization profile image");
    } else if (more && !image2) {
      setIsFailed(true);
      setIsFailedError("Provide your organization cover image");
    } else {
      ...
      setShowNotif(true);
      setCreated(true);
      setCreatedSuccessful("Organization Created successfully");
    }
  };
      ...
      >
        <ScrollView>
          <BackgroundImages
            image={image}
            image2={image2}
            onPressImage={() => CreateOrg.pickImage(setImage, setImage1)}
            onPressImage2={() => CreateOrg.pickImage(setImage2, setImage3)}
          />
