...
import { CreateServ } from "./CreateServicePage";
...
const CreateServicePages = () => {
  ...
  const handleCreate = () => {
    ...
    if (name.length >= 20) {
      setIsFailed(true);
      setIsFailedError("service name too long");
    } else if (
      directoryList.some(
        (name) =>
      ...
      if (user.verified === false) {
        setIsFailed(true);
        setIsFailedError(
          "You are not verified. This service is available for premium accounts, upgrade to premium."
        );
      } else if (name.length <= 5) {
        setIsFailed(true);
        setIsFailedError("Service name too short");
        ...
        setIsFailed(true);
        setIsFailedError("Provide your service profile image");
      } else if (more && !image2) {
        setIsFailed(true);
        setIsFailedError("Provide your service cover image");
      } else {
        CreateServ.handleCreateService(
          user,
        ...
        setCreated(true);
        setCreatedSuccessful("Service Created");
        navigation.goBack();
        setShowNotif(true);
      }
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        image={image}
        image2={image2}
        onPressImage={() => CreateServ.pickImage(setImage, setImage1)}
        onPressImage2={() => CreateServ.pickImage(setImage2, setImage3)}
      />
...
export default CreateServicePages;
