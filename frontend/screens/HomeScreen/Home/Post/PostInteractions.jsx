...

{
  /* <View>
        <KeyboardAvoidingView>
          <UserComments
            CommentsView={<CommentInput minHeight={"auto"} item={item} />}
            StatsCountView={
              <StatsCount
                count={item.comments_count ? item.comments_count : 0}
              />
            }
          />
        </KeyboardAvoidingView>
        ...
        <UserCommentingRB
          ...
          GetView={
            <CommentInput
              minHeight={"auto"}
              setCloseCommenting={setCloseCommenting}
...
const PostInteractions = ({ item, OpenCommentScreen }) => {
  const { theme } = useGlobal();
  let isLight = theme === "light";
  const sharing = useRef([]);

  const user = useGlobal((state) => state.user);
  const postLike = useGlobal((state) => state.postLike);
  const commentLike = useGlobal((state) => state.commentLike);
  const [executeLike, setExecuteLike] = useState(false);
  const [closeCommenting, setCloseCommenting] = useState(false);
  ...
  const handleLike = () => {
    if (item.comments) {
      ...
      commentLike(postId, userId, "like");
      setExecuteLike(true);
  ...
  const postLink = `apost/Post/${item.id}`;
